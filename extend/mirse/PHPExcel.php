...
//                'style' => \PHPExcel_Style_Border::BORDER_MEDIUMDASHDOTDOT,//点虚粗线
            ...
            foreach ($cellkey as $k=>$v){
                foreach ($list as $key=>$val){
                    if(isset($val[$k]) && !is_array($val[$k])){
                        $sheet->setCellValueExplicit($v.(self::$topNumber+1+$key),$val[$k],\PHPExcel_Cell_DataType::TYPE_STRING);
                    }else if(isset($val[$k]) && is_array($val[$k])){
                        $str='';
                        foreach ($val[$k] as $value){
                            $str.=$value.chr(10);
                        }
                        $sheet->setCellValueExplicit($v.(self::$topNumber+1+$key),$str,\PHPExcel_Cell_DataType::TYPE_STRING);
                    }
                }
            }
     ...
     * @return $this
     */
    public function setExcelTile($title='',$Name='',$info=[],$funName=null,$index=0){
        //设置参数
        if(is_array($title)){
        ...
        // 创建sheet
        if($index !== 0) self::$PHPExcel->createSheet();
        self::$PHPExcel->setactivesheetindex($index);
        self::$PHPExcel->getActiveSheet($index)->setTitle($Name);

        self::$PHPExcel ->getActiveSheet($index)->setCellValue('A1', $title);
        //文字居中
        self::$PHPExcel->getActiveSheet($index)->getStyle('A1')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        self::$PHPExcel->getActiveSheet($index)->getStyle('A2')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
        self::$PHPExcel->setActiveSheetIndex($index);
        self::$PHPExcel->getActiveSheet($index)->setTitle($Name);

        self::$PHPExcel->getActiveSheet($index)->setCellValue('A2',self::setCellInfo($info));
        //合并表头单元格
        self::$PHPExcel->getActiveSheet($index)->mergeCells('A1:'.self::$cells.'1');
        self::$PHPExcel->getActiveSheet($index)->mergeCells('A2:'.self::$cells.'2');

        self::$PHPExcel->getActiveSheet($index)->getRowDimension(1)->setRowHeight(40);
        self::$PHPExcel->getActiveSheet($index)->getRowDimension(2)->setRowHeight(20);
        //设置表头行高
        if($funName!==null && is_callable($funName)){
            $fontstyle=self::$PHPExcel->getActiveSheet($index);
            $funName($fontstyle,'A1','A2');
        }else{
            //设置表头字体
            self::$PHPExcel->getActiveSheet($index)->getStyle('A1')->getFont()->setName('黑体');
            self::$PHPExcel->getActiveSheet($index)->getStyle('A1')->getFont()->setSize(20);
            self::$PHPExcel->getActiveSheet($index)->getStyle('A1')->getFont()->setBold(true);
            self::$PHPExcel->getActiveSheet($index)->getStyle('A2')->getFont()->setName('宋体');
            self::$PHPExcel->getActiveSheet($index)->getStyle('A2')->getFont()->setSize(14);
        }
        self::$PHPExcel->getActiveSheet($index)->getStyle('A3:'.self::$cells.'3')->getFont()->setBold(true);
    ...
    public static function exportExcel($expTitle,$expCellName,$expTableData,$sheetName,$info)
    {
        self::setExcelHeader($expCellName);

        foreach ($expTableData as $key => $item) {
            $excel = new PHPExcel();
            $excel->setExcelTile($expTitle, $sheetName[$key], $sheetName[$key] . ' - ' . $info,null, $key);
            $excel->setExcelContent($item);
        }
